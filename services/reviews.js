...
export const insertReviewSVC = async (videogameId, userId, description, stars) => {
    ...
    const insertReview = "INSERT INTO reviews (reviewId, videogameId, userId, description, stars, insert_time) VALUES (?, ?, ?, ?, ?, ?)"
    const results = await databaseExecute(insertReview, [reviewId, videogameId, userId, description, stars, current_time])
